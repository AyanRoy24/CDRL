...
from mods.offline_safety_gymnasium.gym_envs import AntModEnv
from mods.offline_safety_gymnasium.ant_n import AntMod
...
gymnasium_register(
    id='MyAnt-v0',
    entry_point='mods.offline_safety_gymnasium.gym_envs:AntModEnv',
    kwargs={'gravity': -4.905, 'goal_idx': 0}
)
...
@dataclass
class AntModCfg(TrainCfg):
    task: str = "AntMod"
    goal_idx: int = 0
    max_episode_steps: int = 300
    gravity: float = -4.905
    param_dict: Optional[Dict[str, Any]] = field(default_factory=lambda: {
        'fl hip lower limit': [0.01],
        'fl hip upper limit': [0.01],
        'fr hip lower limit': [0.01],
        'fr hip upper limit': [0.01],
    })
...
@dataclass
class AntVelocityV0Cfg(TrainCfg):
...
@dataclass
class HalfCheetahVelocityV0Cfg(TrainCfg):
...
@dataclass
class HopperVelocityV0Cfg(TrainCfg):
...
@dataclass
class Walker2dVelocityV0Cfg(TrainCfg):
...
@dataclass
class SwimmerVelocityV0Cfg(TrainCfg):
...
@dataclass
class AntVelocityV2Cfg(TrainCfg):
...
@dataclass
class HalfCheetahVelocityV2Cfg(TrainCfg):
...
@dataclass
class HopperVelocityV2Cfg(TrainCfg):
...
@dataclass
class Walker2dVelocityV2Cfg(TrainCfg):
    task: str = "OfflineWalker2dVelocityGymnasium-v2"
    cost_start: float = 5
    cost_end: float = 300
    epoch_start: int = 200
...
@dataclass
class SwimmerVelocityV2Cfg(TrainCfg):
...
TASK_TO_CFG = {
    ...
    "OfflineAntVelocityGymnasium-v0": AntVelocityV0Cfg,
    "OfflineHalfCheetahVelocityGymnasium-v0": HalfCheetahVelocityV0Cfg,
    "OfflineHopperVelocityGymnasium-v0": HopperVelocityV0Cfg,
    "OfflineWalker2dVelocityGymnasium-v0": Walker2dVelocityV0Cfg,
    "OfflineSwimmerVelocityGymnasium-v0": SwimmerVelocityV0Cfg,
    "OfflineAntVelocityGymnasium-v2": AntVelocityV2Cfg,
    "OfflineHalfCheetahVelocityGymnasium-v2": HalfCheetahVelocityV2Cfg,
    "OfflineHopperVelocityGymnasium-v2": HopperVelocityV2Cfg,
    "OfflineWalker2dVelocityGymnasium-v2": Walker2dVelocityV2Cfg,
    "OfflineSwimmerVelocityGymnasium-v2": SwimmerVelocityV2Cfg,
...
class ActorProbLargeVar(ActorProb):
    ...
    def forward(
        ...
        # print("Observation space shape:", state.shape)
...
def train(args: TrainCfg):
    ...
    env = gym.make("MyAnt-v0")  # using the registered gymnasium environment
    ...
    if args.collect_in_train:
        ...
        train_envs = worker([lambda: gym.make("MyAnt-v0") for _ in range(training_num)])
    ...
    test_collector = BasicCollector(policy, gym.make("MyAnt-v0"), traj_buffer=traj_buffer)
    ...
    def saving_dataset():
        ...
        traj_buffer.save(dataset_dir) # Saving dataset...
