...
class PointRobot(gym.Env):
    def __init__(self, id=None, seed=None, hazard_position_list=None, hazard_size=None, goal_position=None):
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(11,), dtype=np.float32)
        self.action_space = gym.spaces.Box(low=-1, high=1, shape=(2,), dtype=np.float32)
        # (S, A, T, r) are fixed above; only hazard/goal geometry may differ between
        # a source and a target CMDP, since that is what defines the cost function c.
        self.hazard_size = hazard_size if hazard_size is not None else 0.8
        # self.hazard_size = 1.8
        
        # self.hazard_position_list = [np.array([0.4, -1.2]), np.array([-0.4, 1.2])]
        # self.hazard_position_list = [np.array([1.1, -0.2]), np.array([-1.7, 0.2])] #3
        # self.hazard_position_list = [np.array([1.3, -1.0]), np.array([-1.3, 1.0])] #4
        self.hazard_position_list = (
            hazard_position_list if hazard_position_list is not None else [np.array([0.4, -1.2]), np.array([-0.4, 1.2])]
        )
        self.goal_position = goal_position if goal_position is not None else np.array([2.2, 2.2])
        self.goal_size = 0.3
        self.dt = 0.05
        self.state = None
        self.id = id
